...
%macro downld_apr(db2lib,
    ...
    proc sql;
        ...
        sum(PHS_GRP_REV,PHS_CTRCT_REV,PHS_TRAN_REV)/
        sum(PHS_GRP_RMS,PHS_CTRCT_RMS,PHS_TRAN_RMS) as apr
        from &db2lib..oyt_pj_hist_smry
        where phs_prop_code in (
        %do j = 1 %to &num_pc_lists ;
             %if &j < &num_pc_lists %then ,;
             &&pclist_&j
        %end;
        )
        and phs_inv_date between "&startdt9"D and "&lastdt9"D
        group by prop_code, inv_date

    ...
    ** regression and forecast, run the relevant code section.        **;
    ...
    %if &run_regfcst_yn = Y %then %do;
        ...
        ** Find the list of prop codes to process through regression / forecast. **;
        ...
        proc sort data = db2lref.OYT_PR_RULE_LKUP 
            ...
            where prl_rule_set_id = &pr_rule_set_id
              and prl_use_retail = &use_retail_value ;
            by prl_prop_code;
        run;
        ...
        ** Add the exception property list **;
        ...
        %else %do;
             %let errflg = 2 ;
             %let errmsg = No Properties To Process Through EGP Regression / Forecast ;
        %end;
        ...
        ** Find New Properties list for properties that have been open **;
        ** less than [15] months.                                      **;

        data pclist_0 
             new_props (drop = reg_pc_yn) 
             regression_process_list (drop = reg_pc_yn);
            set pclist_0 ;
            months_open = intck(month, open_date, "last_dt9"d) ;
            reg_pc_yn = 'N';
            if months_open < &new_prop_max_months_open then output new_props;
            else do;
                output regression_process_list ;
                reg_pc_yn = 'Y';
            end;
        ...
        ** Obtain mirror properties of the new properties **;

        proc sql;

            create table mirror_props
            as select rlu.prl_prop_code as prop_code,
                      rlu.prl_retail_mirror_prop as prop_code_mirror
            from new_props np,
                 db2lref.oyt_pr_rule_lkup rlu
            where np.prop_code = rlu.prl_prop_code 
            order by rlu.prl_prop_code ;

       quit;
                  


        ** Remove properties from the mirror_props data set **;
        ** when the mirror property is not found in the EGP **;
        ** process list.                                    **;

        proc sql;

            create table mirror_prop_list2
            as select m.prop_code as orig_prop_code,
                      m.prop_code_mirror
            from mirror_props m,
                 regression_process_list rp
            where m.prop_code_mirror = rp.prop_code ;
        ...
        ** Copy the post-no-mirror-property removed property list to sasdmart **;
        data sasdmart.regression_process_list ;
            set regression_process_list;
        run;
        ...
        ** Write prop_codes to lists of 1000 or so properties **;
        ...
        %do %while (&dataobs > 0);
            ...
            data pclist_&loop ;
                length pcstr pcstr_reg $10000 ;

                set pclist_&lastiter end=eof;

                retain pcstr pcstr_reg ;

                if _n_ > 1 and _n_ <= 1000 then do;
                    pcstr = trimn(pcstr) ||"," ;
                    if length(compress(pcstr_reg)) > 1 then 
                     pcstr_reg = trimn(pcstr_reg) ||"," ;
                end;

                if _n_ <= 1000 then do;
                    pcstr = trimn(pcstr) || compress("'"|| prop_code ||"'");
                    if reg_pc_yn = 'Y' then 
                     pcstr_reg = trimn(pcstr_reg) || compress("'"|| prop_code ||"'");
                end;
                else output ;

                if _n_ = 1000 or eof then do;
                    call symput(compress("pclist_&loop"), trimn(pcstr)) ;
                    call symput(compress("pclist_reg_&loop"), trimn(pcstr_reg)) ;
                    if eof then call symput('num_pc_lists', put(&loop, 4.));
                end; 

                drop pcstr pcstr_reg; 
            run;
        ...
        ** Obtain parameter estimates for mirror properties **;

        data mirror_prop_estimates (rename = (orig_prop_code = prop_code));
            merge parameter_estimates (in = pe_)
                  mirror_prop_list2 (in = mp_ rename = (mirror_property = prop_code)) ;
            by prop_code ;
            if mp_ and pe_ ;
            rename prop_code = mirror_prop_code ;
        run;


        ** Add the parameter estimates taken from mirror properties **;
        ** to the list of parameter estimates to be used.           **;

        data parameter_estimates ;
            set parameter_estimates
                mirror_prop_estimates ;
        run;
